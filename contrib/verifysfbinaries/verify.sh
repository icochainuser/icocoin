...
###   This script attempts to download the signature file SHA256SUMS.asc from icocoin.org
...
WORKINGDIR="/tmp/icocoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://icocoin.org/bin/"
VERSIONPREFIX="icocoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'icocoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Icocoin Core binary release signing key installed?"
   fi
